      ...
      function showBillCsvPreview(rows, m, fname, warnings) {
        ...
        document.getElementById('billCsvPreviewTable').innerHTML =
          ...
          if (c.category) {
            ...
            if (c.category.type === 'currency') {
              ...
              formatted =
                val === 0
                  ? '—'
                  : '$' + val.toLocaleString('en-US', { minimumFractionDigits: 5, maximumFractionDigits: 5 });
        ...
        if (
          !(await confirmAsync(
            'Delete all ' +
              ...
              ' billing period' +
              (count !== 1 ? 's' : '') +
              ' from this meter? This cannot be undone.',
          ))
        )
          return;
      ...
      async function deleteBillRow(mid, rowId) {
        ...
        if (_delRow) {
          const period =
            (_delRow.start || _delRow.BillingPeriodStart || '?') +
            ' to ' +
            (_delRow.end || _delRow.BillingPeriodEnd || '?');
      ...
      function showBillSplitPanel(mid, billId, evt) {
        if (evt && (evt.target.closest('button') || evt.target.closest('select') || evt.target.closest('input')))
          return;
        const b = getUDBldg(udSelProjId, udSelBldgId);
        if (!b) return;
      ...
      function _updateUnitPreview() {
        ...
        preview.textContent =
          '1 ' + billU + ' = ' + converted.toFixed(6).replace(/0+$/, '').replace(/\.$/, '') + ' ' + dispU;
      ...
      function openMeterModal(editId, projId, bldgId) {
        ...
        if (editId) {
          ...
          ['mm-provider', 'mm-account', 'mm-meter', 'mm-maddr'].forEach(
          ...
          t.defaultTopics = [
            ...
            },
            {
              ...
              subItems: [
                ...
                'Upcoming project milestones and dependencies.',
              ],
            },
            {
        ...
        body.innerHTML = `
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
            ...
            <div style="display:flex;gap:6px">
              <button class="btn btn-ghost btn-sm" onclick="openReportModal(${projId},'quarterly')">📊 Generate Report</button>
      ...
      function openMeetingEditor(projId, meetingId) {
          ...
          if (_recur && typeof getNthWeekdayOfMonth === 'function') {
            for (let offset = 0; offset < 3; offset++) {
              ...
              if (meetDate && meetDate >= new Date()) {
                ...
                if (!exists) {
                  meetDate.setHours(
                    parseInt(_recur.time?.split(':')[0]) || 9,
                    parseInt(_recur.time?.split(':')[1]) || 0,
                  );
                  now = meetDate;
                  break;
                }
        ...
        document.getElementById('meetingModalTitle').textContent = meetingId
          ...
          : 'New Meeting Agenda';
        document.getElementById('mtgConvertBtn').style.display =
          _editingMeeting.type === 'agenda' && meetingId ? '' : 'none';
      ...
      function mtgPickContact(tableIdx, contactIdx, pickerValue, isCsc) {
        ...
        const name =
          src.name ||
          ((src.title ? src.title + ' ' : '') + (src.first || '') + (src.last ? ' ' + src.last : '')).trim();
      ...
      function buildMeetingPDF(m, returnDoc) {
        ...
        doc.text(
          (m.sectionHeading || 'Energy Management Services').replace(' Program', '') + ' ' + typeLabel,
          titleX,
          y,
          { align: 'center' },
        );
