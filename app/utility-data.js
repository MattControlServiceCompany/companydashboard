      ...
      function loadUtilityData() {
        ...
        const _ratesMigratedKey = 'en_utility_rates_backfilled_v1';
        if (!localStorage.getItem(_ratesMigratedKey)) {
          ...
          if (ratesFilled > 0) {
            saveUtilityData();
            console.log('[rate backfill] Populated derived rates on ' + ratesFilled + ' bills');
          }
      ...
      function _auditPeriodLabel(row) {
        ...
        if (row.BillingPeriodStart && row.BillingPeriodEnd)
          return row.BillingPeriodStart + ' to ' + row.BillingPeriodEnd;
        return '';
      }
      ...
      function toggleUDProjPanel(key) {
        ...
        content.innerHTML =
          '<div style="padding:20px;font-size:12px;color:var(--text2)">Aggregating project data...</div>';
      ...
      function renderUDProjAggPanel(content) {
        ...
        if (_udProjPanel === 'baseline') {
          ...
          const rows = MN.map((mn, mo) => {
            const e = elecByMo[mo],
              g = gasByMo[mo];
            return `<tr><td style="padding:6px 10px;font-size:12px;font-weight:600;border:1px solid var(--border);background:var(--s1)">${mn}</td>
          ...
          content.innerHTML = `
            <div style="padding:14px 18px;background:var(--s1);border-bottom:1px solid var(--border)">
              ...
              <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px">
                ${totalSqft ? `<div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Total Sq Ft</div><div style="font-size:15px;font-weight:800">${$n(totalSqft)}</div></div>` : ''}
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Annual kWh</div><div style="font-size:15px;font-weight:800;color:var(--em2)">${$n(annKwh)}</div></div>
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Annual Therms</div><div style="font-size:15px;font-weight:800;color:var(--warn)">${$n(annTherms)}</div></div>
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Electric $/yr</div><div style="font-size:15px;font-weight:800;color:var(--em2)">${$f(annElecCost)}</div></div>
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Gas $/yr</div><div style="font-size:15px;font-weight:800;color:var(--warn)">${$f(annGasCost)}</div></div>
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Total Utility $/yr</div><div style="font-size:15px;font-weight:800;color:var(--em)">${$f(annTotal)}</div></div>
                ${kwhRate ? `<div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Avg $/kWh</div><div style="font-size:15px;font-weight:800">${$f(kwhRate, 4)}</div></div>` : ''}
                ${thermRate ? `<div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Avg $/Therm</div><div style="font-size:15px;font-weight:800">${$f(thermRate, 2)}</div></div>` : ''}
                ${eui ? `<div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Site EUI</div><div style="font-size:15px;font-weight:800;color:var(--violet)">${$n(eui, 1)} <span style="font-size:10px;font-weight:400">kBtu/sf/yr</span></div></div>` : ''}
              </div>
            </div>
          ...
          const rows = MN.map((mn, mo) => {
            ...
            return `<tr><td style="padding:6px 10px;font-size:12px;font-weight:600;border:1px solid var(--border);background:var(--s1)">${mn}</td>${tdN(base)}${tdN(base - sav)}${tdN(sav, 'var(--em)')}</tr>`;
          }).join('');
          content.innerHTML = `
            <div style="padding:14px 18px;background:var(--s1);border-bottom:1px solid var(--border)">
              ...
              <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-top:8px">
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Annual Baseline</div><div style="font-size:15px;font-weight:800">${$f(annBase)}</div></div>
                ...
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Projected Spend</div><div style="font-size:15px;font-weight:800;color:var(--em2)">${$f(annBase - annProjSav)}</div></div>
                <div style="background:var(--s2);border:1px solid var(--border);border-radius:7px;padding:8px 11px"><div style="font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Buildings</div><div style="font-size:15px;font-weight:800">${bldgs.length}</div></div>
              </div>
            </div>
            <div style="overflow-x:auto"><table style="border-collapse:collapse;width:100%;font-size:12px">
              <thead><tr style="background:var(--s1)"><th style="${thS};text-align:left">Month</th>
      ...
      const CONDENSED_CATEGORIES = {
        Electric: [
          ...
          { label: 'Facilities kW', type: 'number', w: 100, compute: (r) => _pfBills(r.facKW) },
          {
            ...
            w: 100,
            compute: (r) =>
              _pfBills(r.demandCharge) + _pfBills(r.tdcCharge) + _pfBills(r.facilitiesCharge || r.facKWCost),
          },
          {
        ...
        const tblHead =
          ...
          cols
            .map((c, i) => {
              ...
              return `<th class="${classes.filter(Boolean).join(' ')}" style="min-width:${c.w}px" data-col="${i}">${c.h}<div class="col-resize-handle" data-col="${i}"></div></th>`;
            })
            .join('') +
          `</tr></thead>`;
          ...
          tblBody +=
            ...
            cols
              .map((c, i) => {
                ...
                const dataAttr =
                  (c.sticky ? ' data-sticky="' + i + '"' : '') +
                  (c.rightSticky ? ' data-sticky-right="' + i + '"' : '');
      ...
      function getDateRange(bills) {
        ...
        return (
          fmtMon(sorted[0].start) + ' – ' + fmtMon(sorted[sorted.length - 1].end || sorted[sorted.length - 1].start)
        );
      }
      function fmtMon(d) {
        if (!d) return '?';
      ...
      function _renderExportScopeTree(preProjId, preBldgId, preMeterId) {
        ...
        projs.forEach(function (p) {
          ...
          bldgs.forEach(function (b) {
            ...
            const bldgChecked =
              preBldgId && b.id === preBldgId ? 'checked' : projChecked && !preBldgId ? 'checked' : '';
            html +=
      ...
      function _exportScopeToggleBldg(pid, bid) {
        ...
        document
          .querySelectorAll('.exp-cb-meter[data-pid="' + pid + '"][data-bid="' + bid + '"]')
          .forEach(function (cb) {
            cb.checked = checked;
          });
        _refreshExportFieldsList();
        ...
        try {
          ...
          const tblRows = rows
            .map((r) => {
              ...
              const euiCell = hasEUI
                ? `<td class="mono" style="color:var(--text3)">${(r.usage / sqft).toFixed(4)}</td>`
                : '';
      ...
      function renderBaselinePane(pane, m, bills, incl) {
        ...
        if (hasBl) {
          ...
          const avgRegrBl = regrBlRows.length
            ? regrBlRows.reduce((s, r) => s + r.regrBaseline, 0) / regrBlRows.length
            : null;
          ...
          const pills = [
            ...
            avgRegrBl != null
              ...
              : null,
            cost > 0
              ? { v: '$' + cost.toLocaleString(undefined, { maximumFractionDigits: 0 }), lbl: 'Total Cost' }
              : null,
            ...
            winterRate != null && m.commodity !== 'Propane'
              ...
              : null,
            summerKwRate != null
              ? { v: fmtKw(summerKwRate) + '/kW', lbl: '☀️ Summer $/kW', color: 'var(--warn)' }
              : null,
        ...
        if (isElecBl && hasBl) {
          blRows.forEach((r) => {
            ...
            const bilKW =
              bfr.reduce((s, b) => s + parseFloat(b.billedKW || b.demandKW || 0), 0) / Math.max(1, bfr.length);
      ...
      function refreshBaselineStats(mid) {
        ...
        if (statsEl && hasBl) {
          ...
          const summerRate = sumR.length
            ? sumR.reduce((s, r) => s + eCostByYm2[r.ym] / r.usage, 0) / sumR.length
            : null;
          const winterRate = winR.length
            ? winR.reduce((s, r) => s + eCostByYm2[r.ym] / r.usage, 0) / winR.length
            : null;
      ...
      function saveBaseline(mid) {
        ...
        showToast(
          'Baseline saved — ' +
            ...
            ' months' +
            (_blInherited
              ? ' · ' + _blInherited + ' other meter' + (_blInherited !== 1 ? 's' : '') + ' inherited'
              : '') +
            ' ✓',
        );
      }
      ...
      function drawBlChart(canvasId, rows, unit, bills, incl, showOverlay, metric) {
        ...
        if (rows.length > 12) {
          ...
          rows.forEach((r) => {
            ...
            if (!byMo[mo])
              byMo[mo] = { regrVals: [], usageVals: [], normDaysVals: [], costs: [], label: MON_SHORT[mo], mo };
      ...
      function renderBuildingStatsPane(pane, b) {
        ...
        const { elecByMo } =
          elecM && eBlRows.length ? buildMoMap(elecM, eBlRows, elec.bills, elec.incl) : { elecByMo: {} };
            ...
            options: {
              ...
              plugins: {
                ...
                tooltip: {
                  callbacks: {
                    label: (ctx) =>
                      ctx.parsed.y > 0 ? ' ' + ctx.dataset.label + ': ' + fmtDollar(ctx.parsed.y) : null,
                    footer: (items) => {
          ...
          for (let yr = 0; yr < years; yr++) {
            for (let q = 0; q < 4; q++) {
              ...
              const yms = postStartYM
                ? [
                    addMonths(postStartYM, offset),
                    addMonths(postStartYM, offset + 1),
                    addMonths(postStartYM, offset + 2),
                  ]
                : null;
      ...
      function renderPerfPane(pane, m, bills, incl) {
        ...
        const blThermCostByCalMo = Object.fromEntries(
          Object.entries(meterDataByMo).map(([mo, v]) => [mo, v.thermCost]),
        );
        const blTotalCostByCalMo = Object.fromEntries(
          Object.entries(meterDataByMo).map(([mo, v]) => [mo, v.totalCost]),
        );
        ...
        const rolling12kWh = last12post.reduce(
          (s, r) => s + (rawUsageByYm[r.ym] != null ? rawUsageByYm[r.ym] : r.usage),
          0,
        );
        ...
        const blRegrAvgMo = blRegrRows.length
          ? blRegrRows.reduce((s, r) => s + r.regrBaseline, 0) / blRegrRows.length
          : null;
        ...
        if (isElec) {
          filteredPostRows.forEach((r) => {
            ...
            perfDemByYm[r.ym] = {
              ...
              kwRate:
                _storedKwRate || (bilKW > 0 && kwCostAmt + facKWCostAmt > 0 ? (kwCostAmt + facKWCostAmt) / bilKW : 0),
            };
          });
        }
        ...
        const statsHTML =
          ...
          (savCost !== null
            ? blPill(_savFmtPill(savCost), 'Cost Delta', savCost >= 0 ? 'var(--em)' : 'var(--danger)')
            : '') +
            ...
            return (
              ...
              (hasGasCostSav
                ? '<td class="mono num" style="color:' +
                  ...
                  ';font-weight:600">' +
                  (thermRate > 0 || _rowCostOvr != null
                    ? costSign(thermCostSav) + '$' + fmtC(thermCostSav)
                    : '&mdash;') +
                  '</td>'
                : '') +
              (hasExpKW
