...
function dcExtractEvents(text) {
  ...
  const DATE_PATS = [
    ...
    new RegExp(`\\b(${MONTHS_LONG}|${MONTHS_SHORT})\\.?[ \\t\\n\\r]+([0-9]{1,2}),?[ \\t\\n\\r]+([0-9]{4})\\b`, 'gi'),
...
function dcRenderEventList(filter = '') {
  ...
  const filtered = fl ? dcEvents.filter((e) => e.name.toLowerCase().includes(fl) || e.type.includes(fl)) : dcEvents;
...
function distCalDeleteAll(projId) {
  ...
  if (
    !confirm(
      'Are you sure you want to delete all ' + count + ' calendar events for this project? This cannot be undone.',
    )
  )
    return;
  p.districtCalendar = [];
  ...
  for (const line of eventLines) {
    ...
    if (headers.length >= 2) {
      ...
      for (let c = 0; c < headers.length; c++) {
        const start = c === 0 ? 0 : Math.floor((headers[c - 1].pos + headers[c - 1].name.length + headers[c].pos) / 2);
...
function openEquipModal() {
  if (typeof initQuillEditor === 'function') {
    if (!window._eqNotesQuill) {
      window._eqNotesQuill = initQuillEditor('eq-notes-editor', '');
    } else {
      window._eqNotesQuill.setContents([]);
    }
  }
...
function saveEquip() {
  ...
  equipment.push({
    ...
    notes:
      typeof getQuillHTML === 'function' && window._eqNotesQuill
        ? getQuillHTML(window._eqNotesQuill)
        : document.getElementById('eq-notes-editor')
          ? document.getElementById('eq-notes-editor').textContent
          : '',
  });
  ...
  updateHomeStats();
  showToast(tag + ' added ✓');
  ['eq-tag', 'eq-make', 'eq-model', 'eq-cap', 'eq-eff', 'eq-loc'].forEach(
    (id) => (document.getElementById(id).value = ''),
  );
  if (window._eqNotesQuill) window._eqNotesQuill.setContents([]);
}
