      ...
      function validateBillData(extracted, utilityName) {
        ...
        if (_extComm === 'gas') {
          ...
          if (gasTotal !== 0) {
            const gasCompSum =
              Math.round(
                (pf(extracted.CustomerCharge) + pf(extracted.GasCharge) + pf(extracted.FuelAdjustment)) * 100,
              ) / 100;
            if (gasCompSum > 0) {
        ...
        const isPropaneBill =
          (extracted.Commodity || '').toLowerCase() === 'propane' || (extracted.FuelType || '') !== '';
      ...
      function detectStatisticalOutliers(extracted) {
        ...
        if (totalVal > 0) {
          const totalStats = getStats(historicalBills, (b) =>
            pf(b.totalCost || b.TotalCurrentCharges || b.TotalAmountDue),
          );
          if (totalStats) {
      ...
      function _postExtractionVerify(bills, utilityName, rawText) {
            ...
            if (_gc > 0 && _th > 0) {
              ...
              if (origRate && origRate.severity === 'error') {
                ...
                const best = candidates
                  ...
                  .sort(
                    (a, b) => Math.abs(a.rate.implied - a.rate.typical) - Math.abs(b.rate.implied - b.rate.typical),
                  )[0];
                if (best) {
                  if (best.gc !== _gc) {
        ...
        if (utilityName === 'Evergy') {
          for (let i = 0; i < bills.length; i++) {
            ...
            const kwhOffFromConsensus =
              consensusKwh && curKwh > 0 && Math.abs(curKwh - consensusKwh) / consensusKwh > 0.05;
            ...
            b._auto_corrected_TotalCurrentCharges = {
              ...
              reason:
                violations
                  .map(function (vi) {
                    return (
                      vi.field + ' ($' + vi.value.toFixed(2) + ') exceeded total ($' + pf(origTotal).toFixed(2) + ')'
                    );
                  })
                  .join('; ') +
            ...
            b._charge_exceeds_total = {
              ...
              reason:
                violations
                  .map(function (vi) {
                    return (
                      vi.field + ' ($' + vi.value.toFixed(2) + ') > TotalCurrentCharges ($' + total.toFixed(2) + ')'
                    );
                  })
      ...
      async function confirmAutoAssign() {
        ...
        if (overrideRow && overrideRow.style.display !== 'none') {
          ...
          if (overrideProjId && overrideBldgId && overrideMeterId) {
            ...
            const overrideMeter = overrideBldg
              ? (overrideBldg.meters || []).find((m) => m.id === overrideMeterId)
              : null;
          ...
          if (bill.AccountNumber && targetMeter.account) {
            ...
            if (_extN !== _stoN && (_extN.includes(_stoN) || _stoN.includes(_extN))) {
              if (bill.AccountNumber.length > targetMeter.account.length) {
                console.log(
                  '[_saveBillToMatchedMeter] updating meter account',
                  targetMeter.account,
                  '→',
                  bill.AccountNumber,
                );
        ...
        if (match) {
          return {
            ...
            destination:
              ...
              match.bldg.name +
              ' → ' +
              (match.meter.provider || match.meter.meter || 'meter'),
            match,
          };
        }
        if (projId) {
      ...
      function renderQueueProgress() {
        ...
        const pct =
          q.files.length > 0
            ? Math.round(
                ((q.status === 'done' || q.status === 'cancelled' ? q.files.length : q.currentIdx) / q.files.length) *
                  100,
              )
            : 0;
        ...
        q.files.forEach((f, i) => {
          ...
          if (result && result.status === 'ok') {
            ...
            html +=
              '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:13px;color:var(--em)">';
            html +=
            ...
            html += '<span>' + _escHtml(f.name) + '</span>';
            html +=
              '<span style="font-size:11px;color:var(--text)"> — ' + (q._fileStatus || 'extracting...') + '</span>';
            html += '</div>';
          } else {
            html +=
        ...
        return new Promise((resolve, reject) => {
          ...
          reader.onload = async (ev) => {
            try {
              ...
              if (
                finalBills.length === 0 ||
                (!finalBills[0].BillingPeriodStart &&
                  !finalBills[0].TotalCurrentCharges &&
                  !finalBills[0]._manualReview)
              ) {
      ...
      function renderQueueResults() {
        ...
        if (dupCount > 0)
          html += (okCount > 0 ? ' · ' : '') + '<span style="color:var(--em)">' + dupCount + ' dup</span>';
        if (failCount > 0)
      ...
      async function _checkDuplicates(bills) {
        ...
        const periodClose = (s1, e1, s2, e2) =>
          ...
          !!s2 &&
          !!e2 &&
          dayDiff(s1, s2) <= FUZZY_DAY_TOLERANCE &&
          dayDiff(e1, e2) <= FUZZY_DAY_TOLERANCE;
                  ...
                  logger: (m) => {
                    if (statusCb) {
                      if (m.status === 'loading tesseract core') statusCb('Loading OCR engine...');
                        ...
                        statusCb('Loading English language data...');
                    }
                  },
                },
                {
              ...
              try {
                const result = await Promise.race([
                  ...
                  new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('OCR timeout after 90s')), OCR_TIMEOUT_MS),
                  ),
                ]);
                clearTimeout(timer);
            ...
            for (let idx = 0; idx < ocrNeeded.length; idx++) {
              ...
              for (let pass = 0; pass < OCR_PASSES.length; pass++) {
                ...
                try {
                  ...
                  if (statusCb)
                    statusCb('OCR failed on page ' + pgNum + ' pass ' + (pass + 1) + ': ' + pageErr.message);
                }
      ...
      async function processPDF(file) {
        ...
        reader.onload = async (ev) => {
          ...
          if (pdfType === 'utility') {
            const statusMsg = (msg) => {
              box.innerHTML =
                '<div class="ai-thinking"><div class="tdots"><span></span><span></span><span></span></div> ' +
                msg +
                '</div>';
                    ...
                    for (const scale of RETRY_SCALES) {
                      ...
                      if (retryFull.trim().length > 100) {
                        ...
                        if (retryRule) {
                          ...
                          const retryMissing = Math.max(
                            ...retryCheck.map((b) => countCriticalMissing(b, retryRule.name)),
                          );
                          ...
                          if (retryCheck.length === retryBills.length) {
                            ...
                            // Recount after merge
                            const mergedMissing = Math.max(
                              ...retryBills.map((b) => countCriticalMissing(b, rule.name)),
                            );
      ...
      function renderMultiBillUI(bills, box) {
        ...
        const filteredPeriods =
          activeCommTab === 'All'
            ? periods
            : periods.filter((p) => (bills[p.i].Commodity || 'Other') === activeCommTab);
        ...
        const _buildPill = (p) => {
          ...
          const _chargeSum = (_pillChargeKeys[_pillComm] || _pillChargeKeys.Electric).reduce(
            (s, f) => s + _pf2(bill[f]),
            0,
          );
          ...
          const gapRow = gapInfo
            ...
            : '';
          return (
            `<div class="ef-pill-row"><div class="ef-pill-month">${p.monthLbl || ''}</div>${pillBtn}</div>` + gapRow
          );
        };
        let nav;
        if (_multiAcct) {
          ...
          const _bldgName = (acct) => {
            ...
            return (
              afterComma
                .replace(/\b(LOUISBURG|KANSAS CITY|OLATHE|LENEXA|OVERLAND PARK|SHAWNEE|KS|MO)\b/gi, '')
                .trim() || acct
            );
          };
      ...
      function renderPDFFields(parsed, warnings) {
        ...
        function buildCell(k) {
          ...
          if (isDupDiff)
            borderStyle =
              'border-left:3px solid #f59e0b;padding-left:8px;background:rgba(245,158,11,.08);border-radius:4px';
          ...
          const acInlineIcon = _inlineLevel
            ...
            : '';
          const _popClass =
            _popLevel && _popLevel !== 'ac' && !(autoCorrect || ocrConsensus) ? ` level-${_popLevel}` : '';
        ...
        const _billCommodity =
          ...
          (parsed.kWhConsumed
            ...
            : parsed.NaturalGasTherms || parsed.NaturalGasCCF || parsed.GasCharge
              ? 'Gas'
              : 'Electric');
        const _CHARGE_SUM_KEYS_RPF =
          _CHARGE_SUM_KEYS_BY_COMMODITY[_billCommodity] || _CHARGE_SUM_KEYS_BY_COMMODITY.Electric;
        ...
        if (errorCount || warnCount) {
          const parts = [];
          if (errorCount)
            parts.push('<span style="color:var(--red);font-weight:700">' + errorCount + ' missing</span>');
          if (warnCount)
            parts.push(
          ...
          if (infoCount)
            parts.push(
              '<span style="color:var(--accent)">' + infoCount + ' note' + (infoCount > 1 ? 's' : '') + '</span>',
            );
        ...
        if (_liveDup) {
          ...
          const actLabel = _liveDup.action
            ? ` &middot; <strong style="color:var(--em)">${_liveDup.action}</strong>`
            : '';
      ...
      function mam_populateMeters() {
        ...
        meterSel.innerHTML =
          ...
          meters
            .map((m) => {
              const lbl = [m.commodity, m.provider, m.account ? '#' + m.account : '', m.maddr]
                .filter(Boolean)
                .join(' ');
      ...
      async function _saveSinglePDFBill(extracted, projId) {
        ...
        if (totalCost > 0 && diff > 0.1 && !isGas && !isPropane) {
          console.warn(
            'Bill validation: components $' + componentSum.toFixed(2) + ' vs total $' + totalCost.toFixed(2),
          );
        }
        return true;
      }
