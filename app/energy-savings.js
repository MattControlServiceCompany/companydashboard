      ...
      function calcProjSavingsMatrix(projId) {
        ...
        sd.measures.forEach((m) => {
          const row =
            document.getElementById('sv-msr-row-' + projId + '-' + m.id) ||
            document.getElementById('sv-pg-msr-row-' + m.id);
          if (!row) return;
        ...
        sd.measures.forEach((m) => {
          ...
          m.totalDollar = total;
          const cell =
            document.getElementById('sv-msr-total-' + projId + '-' + m.id) ||
            document.getElementById('sv-pg-total-' + m.id);
      ...
      function _extractEvergy(t, acctOverride, addrOverride) {
        ...
        const CHG_STOP =
          /(?:Cust|Fac\S|Demand|Energy\s+C|\bECA|\bEER|\bPTS|\bTD[CG]|\bRkVA|Subtotal|Current\s+Charges)/i;
        ...
        const adjKwh = (() => {
          ...
          // Last resort
          const fallback =
            t.match(/kWh\s+(?:Used|Consumed)[^\d]*([\d,]+\.\d+)/i) ||
            t.match(/([\d,]+\.\d+)\s*kWh\s+(?:Used|Consumed)/i);
          if (fallback) {
        ...
        const result = {
          ...
          MeterMultiplier:
            _meterCombined?.multiplier || meterRow?.[7]?.replace(/,/g, '') || _mlMeter?.multiplier || null,
          kWhConsumed: adjKwh,
        ...
        if (_meterCombined) {
          ...
          if (_meterCombined.type === 'meter_change') {
            result._meterInfo.note =
              'Meter change: ' + _meterCombined.rows + ' meter lines summed for kWh, max KW/RKVA used';
        ...
        for (const [chargeField, ri] of Object.entries(_rates)) {
          ...
          const currentVal = result[chargeField]
            ? parseFloat(String(result[chargeField]).replace(/[$,\s]/g, '')) || 0
            : 0;
        ...
        if (result._rates) {
          ...
          if (!result._rates.TDCCharge && result.TDCCharge) {
            ...
            const tdcRateM = t.match(
              /TD[CG]\s+(?:Ch[gaq9]|C[HhNnRr][Gg]|Gh[gq9])[\s\S]{0,100}?\$?([\d.]+)\s*per\s+k[Ww]/i,
            );
            if (tdcRateM) {
        ...
        if (
          !result.EnergyOnPeakCharge &&
          result.EnergyOffPeakCharge &&
          result.kWhConsumed &&
          _rates.EnergyOffPeakCharge
        ) {
        ...
        if (_total > 0 && Math.abs(_compSum - _total) > 1) {
          ...
          if (bdIdx >= 0 && stIdx > bdIdx) {
            ...
            for (const uc of uncaptured) {
              ...
              else if (!result.TaxExemptDelivery && /tax.*exempt/i.test(lcLine))
                result.TaxExemptDelivery = uc.val.toFixed(2);
              else if (!result.BillOffset && /bill.*offset|offset/i.test(lcLine))
                result.BillOffset = (-uc.val).toFixed(2);
        ...
        if (_recompTotal > 0 && _recompGap > 0.5) {
          ...
          if (target) {
            ...
            result['_auto_recovered_' + target] = {
              ...
              reason:
                'Inferred from sum gap: total $' + _recompTotal.toFixed(2) + ' - charges $' + _recompSum.toFixed(2),
            };
          }
        }
      ...
      const _EVG_BILLING_DETAILS =
        /B[il1]{2}[il1]ng\s+D[ec]t[ao][il1]{1,2}[s5]?\s*[-\u2013\—]\s*[s5]erv[il1]ce\s+from/i;
      ...
      function _lbg_correctGasCharge(gasCharge, gasUsage, totalCharge, billDate) {
        ...
        if (!isNaN(reinterpretedCharge) && Math.abs(reinterpretedCharge - expectedCharge) < 1.0) {
          ...
          return {
            ...
            corrected: true,
            reason:
              'OCR decimal fix: ' + gasCharge + ' → ' + reinterpretedCharge.toFixed(2) + ' (matches rate × usage)',
          };
        }
        if (expectedCharge > 0) {
      ...
      const UTILITY_RULES = [
          ...
          extractAll: (t) => {
            ...
            if (uniqueBills.length > 0) {
              validSections = uniqueBills.map((b, i) => {
                ...
                const prevLatest =
                  i > 0 ? Math.max(...(uniqueBills[i - 1]._groupIdxs || [uniqueBills[i - 1].idx])) : null;
                const nextEarliest =
              ...
              for (const frag of raw) {
                ...
                const isBillingHeader =
                  _EVG_BILLING_DETAILS.test(frag.trim()) || /^Billing\s+Details/i.test(frag.trim());
        ...
        },
        {
          ...
          extract: function (t) {
            ...
            const _readDiffM = t.match(
              /(?:difference|usage|consumption|used)[\s:]*([0-9,]+)(?:\s*(?:ccf|therms?|mcf))?/i,
            );
            // Therms conversion factor
            const _thermFactorM = t.match(
              /(?:therm\s*factor|conversion\s*factor|btu\s*factor|multiplier)[\s:]*([0-9.]+)/i,
            );
            return {
              ...
              NaturalGasTherms:
                t
                  .match(/(?:therms?\s*used|total\s*therms?|gas\s*usage)[\s:]*([0-9,]+\.?\d*)/i)?.[1]
                  ?.replace(/,/g, '') ||
                t.match(/([0-9,]+\.?\d*)\s*therms?/i)?.[1]?.replace(/,/g, '') ||
                null,
              NaturalGasCCF:
                t.match(/([0-9,]+\.?\d*)\s*(?:ccf|hundred\s*cubic\s*feet)/i)?.[1]?.replace(/,/g, '') || null,
        ...
        },
        {
          ...
          extract: function (page) {
            ...
            if (/\d{1,2}\/\d{1,2}\/\d{2,4}/.test(page) && /\$?\d[\d,]*\.\d{2}/.test(page)) {
              ...
              if (r2 && (Array.isArray(r2) ? r2 : [r2]).some((b) => b.TotalAmountDue || b.TotalCurrentCharges))
                return r2;
            }
          ...
          _extractOld: function (page) {
            ...
            if (!AccountNumber)
              AccountNumber = cleaned.match(/Account\s*#?\s*:?\s*(\d{5,8})\s+Bill\s*Date/i)?.[1] || null;
            if (!BillDate) BillDate = cleaned.match(/Bill\s*Date[:;]?\s*(\d{1,2}\/\d{1,2}\/\d{2,4})/i)?.[1] || null;
            if (!ServiceAddress)
              ServiceAddress =
                cleaned
                  .match(/Service\s*Address:?\s+([A-Z0-9][A-Z0-9 .,]+?)(?:\s+Account|\s+Amount|\n)/i)?.[1]
                  ?.trim() || null;
            ...
            for (const ln of lines) {
              ...
              else if (/\bW[A4]TER\b/i.test(trim) && !/PROTECTION|STORM/i.test(trim) && !waterLine) waterLine = trim;
            }
            ...
            const water = parseLine(waterLine);
            ...
            const TotalAmountDue =
              cleaned
                .match(/Amount\s*due\s*(?:on\s*or\s*before)?[^$\n]*\$\s*([\d,]+\.\d{2})/i)?.[1]
                ?.replace(/,/g, '') ||
          ...
          _extractNew: function (page) {
            ...
            const parseMetered = (raw, labelRe) => {
              ...
              // labels and ignored.
              const allBefore = [...before.matchAll(/-?[\d,]+(?:\.\d+)?/g)].map((x) =>
            ...
            for (const ln of lines) {
              ...
              if (!water && /\bM?W[A4]TER\b/i.test(ln) && !/PROTECTION/i.test(ln))
                water = parseMetered(ln, /\bM?W[A4]TER\b/i);
            ...
            let TotalAmountDue =
              page.match(/Total\s*Amount\s*Due\s*\$?\s*([\d,]+\.\d{2})/i)?.[1]?.replace(/,/g, '') || null;
            if (!TotalAmountDue) {
          ...
          _extractBillingDetail: function (page) {
            ...
            for (const ln of lines) {
              ...
              if (code === 'GS' || code === 'LGS') {
                ...
                waterAmt = amt;
                if (consumption && consumption > 0 && consumption < 10000000) waterUsage = consumption;
              }
            }
          ...
          _extractLGSDetail: function (page) {
            ...
            const periodMatch = page.match(
              /service\s*from\s*(\d{1,2}\/\d{1,2}\/\d{4})\s*to\s*(\d{1,2}\/\d{1,2}\/\d{4})/i,
            );
            ...
            if (!totalAmt) {
              const allDollars = [...page.matchAll(/\$\s*([\d,]+\.\d{2})/g)].map((m) =>
        ...
        },
        {
          ...
          detect: (t) =>
            /\bpropane\b|\blp\s*gas\b|\bfuel\s*oil\b|\bmfa\s*oil\b/i.test(t) &&
            /net\s*(?:due|delivery)|invoice/i.test(t),
          extractAll: function (t) {
        ...
        },
        {
          ...
          extract: (t) => ({
            UtilityCompany:
              t.match(/^([A-Z][A-Za-z &]+(?:Electric|Gas|Energy|Utilities?|Power))/m)?.[1]?.trim() || null,
            AccountNumber: t.match(/account\s*(?:number|#|no\.?)[\s:]*([0-9\-]{6,20})/i)?.[1] || null,
            ServiceAddress: t.match(/(?:service|billing)\s*address[\s:\n]+([^\n]{10,60})/i)?.[1]?.trim() || null,
            BillingPeriodStart:
              t.match(/(?:from|period\s*(?:start|from)|billing\s*from)[\s:]*(\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4})/i)?.[1] ||
              null,
            BillingPeriodEnd:
