      ...
      async function uiCustomResetAll() {
        if (!(await confirmAsync('Reset all UI customizations (fonts, layout, colors)? This cannot be undone.')))
          return;
      ...
      function siteBuildSettingsModal() {
        ...
        var fontOpts = UI_CUSTOM_SAFE_FONTS.map(function (f) {
          return (
            '<option value="' + f + '"' + ((ucf.bodyFont || 'Outfit') === f ? ' selected' : '') + '>' + f + '</option>'
          );
        }).join('');
        var headFontOpts = UI_CUSTOM_SAFE_FONTS.map(function (f) {
          return (
            '<option value="' + f + '"' + ((ucf.headFont || 'Outfit') === f ? ' selected' : '') + '>' + f + '</option>'
          );
        }).join('');
      ...
      async function siteResetAllMeterTableSettings() {
        if (!(await confirmAsync('Reset all individual meter table settings? Meters will use the site default.')))
          return;
        var toRemove = [];
        for (var i = 0; i < localStorage.length; i++) {
          var k = localStorage.key(i);
