      ...
      function _hvlRevRenderKw(wrap, projId, bldgId, rd, actualKw, avgKwRate) {
        ...
        const moHdr = MO_SHORT.map(
          (m, i) =>
            `<th style="text-align:center;width:52px;font-size:10px;border-left:1px solid var(--border)">${m}</th>`,
        ).join('');
        ...
        const matchRow = MO_SHORT.map((_, i) => {
          ...
          const color =
            pct >= 90 && pct <= 110 ? 'var(--green)' : pct >= 75 && pct <= 125 ? 'var(--amber)' : 'var(--danger)';
      ...
      function hvacLoadCalc(projId) {
        ...
        const bldgOpts = udBldgs
          .map(
            (b) => `<option value="${b.id}"${b.id === _hvlSelBid ? ' selected' : ''}>${b.name || 'Building'}</option>`,
          )
          .join('');
      ...
      function openSolarCalc(projId) {
        ...
        document
          .querySelectorAll(
            '#sc-main-wrap .sc-inp, #sc-main-wrap .sc-bl, #sc-main-wrap .sc-rate, #sc-main-wrap .sc-prod',
          )
          .forEach((inp) => {
      ...
      function _scUpdateArrayDerived() {
        document.querySelectorAll('.sc-arr-kw').forEach((kwI) => {
          ...
          for (let mi = 0; mi < 12; mi++) {
            const cf =
              parseFloat(document.querySelector(`.sc-arr-capfactor[data-ai="${ai}"][data-mi="${mi}"]`)?.value) || 0;
            ...
            const prod =
              parseFloat(document.querySelector(`.sc-arr-prod[data-ai="${ai}"][data-mi="${mi}"]`)?.value) || 0;
      ...
      function _scDoCalc(projId) {
        ...
        function calcStd(i, kwh, akw, offpk, bkw) {
          ...
          const ptS = bPtsS[i] || R.pts,
            ptE = bPtsE[i] || R.pts;
          const ePts =
            ptS === ptE ? +(kwh * ptS).toFixed(2) : +(pr.sKwh * ptS).toFixed(2) + +(pr.eKwh * ptE).toFixed(2);
        ...
        function effTbl() {
          ...
          const sDR =
            sR.reduce((a, r) => a + r.sk, 0) > 0
              ? sR.reduce((a, r) => a + r.dS, 0) / sR.reduce((a, r) => a + r.sk, 0)
              : 0;
          const wDR =
            wR.reduce((a, r) => a + r.sk, 0) > 0
              ? wR.reduce((a, r) => a + r.dS, 0) / wR.reduce((a, r) => a + r.sk, 0)
              : 0;
          const sUR =
      ...
      function _bcDoCalc(projId) {
        ...
        for (let m = 0; m < 12; m++) {
          for (let bi = 0; bi < BAS_TEMP_BINS.length; bi++) {
            ...
            const newOACoolMbtu = Math.max(
              0,
              binTemp > newCoolOcc ? (1.08 * oaCfm * (binTemp - newCoolOcc)) / 1000 : 0,
            );
            const newOAHeatMbtu = Math.max(
              0,
              binTemp < newHeatOcc ? (1.08 * oaCfm * (newHeatOcc - binTemp)) / 1000 : 0,
            );
