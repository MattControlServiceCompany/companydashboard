      ...
      function _pvRenderMeterPerf(m, bills, incl) {
        ...
        var _td =
          'padding:6px 10px;text-align:right;font-family:var(--mono);font-size:12px;border:1px solid var(--border)';
        var rows = postRows
          .map(function (r) {
      ...
      function egfxRefresh(projId) {
        ...
        if (curSavEl) {
          if (hasEgfxSav) {
            ...
            curSavEl.textContent =
              (totalEgfxSav >= 0 ? '' : '-') + '$' + Math.abs(Math.round(totalEgfxSav)).toLocaleString();
        ...
        function _drawYoyChart(canvasId, field, unit, blAvgArr) {
          ...
          if (!canvas) return;
          if (_maCharts[canvasId]) _maCharts[canvasId].destroy();

          const palette = _yrPalette[field] || _yrPalette['kwh'];
          const datasets = [];
          yrsToShow.forEach((y, i) => {
            const isBl = blYears.has(y);
            // Each year gets a distinctly different shade from the commodity palette.
            // yrsToShow is sorted oldest→newest; i=0 is the oldest (darkest), last is newest (lightest).
            const paletteIdx = Math.max(0, palette.length - 1 - i);
            const barColor = palette[paletteIdx] || palette[0];
            datasets.push({
              label: y + (isBl ? ' (BL)' : ''),
              data: months.map((_, mi) => yearData[y]?.[field]?.[mi] || 0),
          ...
          if (blAvgArr) {
            datasets.push({
              ...
              data: blAvgArr.map((v) => (v > 0 ? v : null)),
              type: 'line',
              borderColor: _blLineColor,
              borderWidth: 2,
              borderDash: [6, 3],
              pointRadius: 0,
              fill: false,
      ...
      */
      ...
      function spParseCSV(text, projId, bldgId, fileName) {
        ...
        showToast(
          'Imported ' + imported.length + ' zone' + (imported.length !== 1 ? 's' : '') + ' — click Save to keep',
        );
        renderSetpointsTab(projId);
      }
      ...
      function _spParseMultiBldgSetpoints(lines, headers, splitRow, projId, colBldgName, headerIdx) {
        ...
        var msg =
          'Imported ' + totalZones + ' zones across ' + matched + ' building' + (matched !== 1 ? 's' : '') + ' ✓';
      ...
      function _basRenderAnalysis(buildings) {
        ...
        function row(label, val, unit) {
          ...
          return (
            '<div class="bas-sum-row"><span class="lbl">' +
            label +
            '</span><span class="val">' +
            display +
            '</span></div>'
