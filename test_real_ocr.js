...
if (rawStart < 0) { console.error('No RAW OCR TEXT marker found'); process.exit(1); }
...
const splitRe = /(?=(?:B[il1]{2}[il1]ng\s+D[ec]t[ao][il1]{1,2}[s5]?\s*[-\u2013\u2014]\s*[s5]erv[il1]ce\s+from|Billing\s+Details\s*[-\u2013\-]\s*service\s+from))/gi;
const raw = rawText.split(splitRe);
const getDP = s => { const m = s.match(_EVG_SF) || s.match(/service\s+from\s+(\d{2}\/\d{2}\/\d{4})\s+to\s+(\d{2}\/\d{2}\/\d{4})/i); return m ? m[1] + '|' + m[2] : null; };
const sections = [];
...
const acct = rawText.match(/[Aa]ccount\s+(?:N[ou]mber\s*)?[:\s\u00a9\u00ae]\s*(\d[\d ]{4,18}\d)/m)?.[1]?.replace(/\s/g, '') || null;
...
const criticalFields = ['CustomerCharge', 'FacilitiesCharge', 'BilledKWCharge',
  'EnergyOnPeakCharge', 'TDCCharge', 'ECACharge', 'TotalCurrentCharges'];
...
for (let i = 0; i < bills.length; i++) {
  ...
  const calcSum = pf(b.CustomerCharge) + pf(b.FacilitiesCharge) + pf(b.BilledKWCharge)
    + pf(b.EnergyOnPeakCharge) + pf(b.EnergyOffPeakCharge) + pf(b.RkVACharge)
    + pf(b.ECACharge) + pf(b.EERCharge) + pf(b.PTSCharge) + pf(b.TDCCharge)
    + pf(b.TaxExemptDelivery) + pf(b.BillOffset) + pf(b.FranchiseFee);
  const total = pf(b.TotalCurrentCharges);
  const diff = Math.abs(calcSum - total);
  const sumMismatch = total > 0 && diff > 0.10;

  const taxExempt = pf(b.TaxExemptDelivery);
  const billOffset = pf(b.BillOffset);
  const offsetMismatch = taxExempt > 0 && Math.abs(Math.abs(billOffset) - taxExempt) > 0.10;

  const issues = [];
  if (nullCritical.length) issues.push(`NULL critical: ${nullCritical.join(', ')}`);
  if (sumMismatch) issues.push(`Sum mismatch: calc=${calcSum.toFixed(2)} vs total=${total.toFixed(2)} (diff=$${diff.toFixed(2)})`);
  ...
  if (issues.length) {
    ...
    console.log(`  Cust=${b.CustomerCharge} Fac=${b.FacilitiesCharge} Dem=${b.BilledKWCharge} OnPk=${b.EnergyOnPeakCharge} OffPk=${b.EnergyOffPeakCharge} RkVA=${b.RkVACharge}`);
    console.log(`  ECA=${b.ECACharge} EER=${b.EERCharge} PTS=${b.PTSCharge} TDC=${b.TDCCharge} TaxEx=${b.TaxExemptDelivery} Offset=${b.BillOffset} Fran=${b.FranchiseFee} Total=${b.TotalCurrentCharges}`);
    console.log('');
  } else {
