...
function buildMeterPerfTableHTML(m, bills, incl, opts) {
  ...
  postRows.forEach(function (r) {
    ...
    // $0.00. Mirrors the same predicate computations/savings.js getMeterSavings() now uses
    // (single source of truth for WHEN a month counts as incomplete — this file mirrors the
    // predicate only, not the $ math, since it independently derives its own per-row rates).
    var _rateIncomplete = false;
    var _rateReason = '';

    var kwhSaved = sav;
    var kwhCostSav = _hasContract && hasKwhCostSav && kwhRate > 0 ? kwhSaved * kwhRate : 0;
    if (hasKwhCostSav && !(kwhRate > 0) && rawUsage > 0) {
    ...
    if (hasKwCostSav && blExpKW > 0 && bilKW > 0 && !(moKwRate > 0)) {
      _rateIncomplete = true;
    ...
    if ((isGas || isPropane) && !(thermRate > 0) && (rawUsage > 0 || (gasMo && gasMo.thermCostAmt > 0))) {
      _rateIncomplete = true;
      _rateReason = (isPropane ? 'propane' : 'gas') + ' $/' + costUnitLabel.toLowerCase() + ' rate unavailable';
    }
