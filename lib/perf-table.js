...
function buildMeterPerfTableHTML(m, bills, incl, opts) {
  ...
  var weatherByYm = typeof getWeatherForBuilding === 'function' ? getWeatherForBuilding().byYm || {} : {};
  ...
  var _moMap = isElec ? eMo : isGas ? gMo : isPropane ? pMo : {};
  ...
  if (isElec) {
    bills.forEach(function (b) {
      ...
      var rate = parseFloat(b.totalKwhRate) || 0;
  ...
  if (isGas || isPropane) {
    bills.forEach(function (b) {
      ...
      var rate = isGas ? parseFloat(b.totalGasRate || 0) : parseFloat(b.totalPropaneRate || 0);
  ...
  // Table classes
  var tblClass = mode === 'tab' ? 'ma-tbl' : 'rpt-table';
  var numClass = mode === 'tab' ? 'mono num' : 'rpt-n';
  var lblClass = mode === 'tab' ? 'lbl' : '';
  var fontSize = mode === 'report' ? 'font-size:7px;' : '';
  ...
  var hdr =
    ...
    numClass +
    '">%</th>';
  if (hasKwhCostSav) hdr += '<th class="' + numClass + '">Actual $/kWh</th>';
  if (hasKwhCostSav) hdr += '<th class="' + numClass + '">kWh Savings ($)</th>';
  if (hasGasCostSav) hdr += '<th class="' + numClass + '">Actual $/' + costUnitLabel + '</th>';
  if (hasGasCostSav) hdr += '<th class="' + numClass + '">' + costUnitLabel + ' Savings ($)</th>';
  if (hasExpKW) hdr += '<th class="' + numClass + '">Baseline kW</th>';
  if (hasKwData) hdr += '<th class="' + numClass + '">Actual kW</th>';
  if (hasBilKW) hdr += '<th class="' + numClass + '">Billed kW</th>';
  if (hasKwCostSav) hdr += '<th class="' + numClass + '">Actual $/kW</th>';
  if (hasKwCostSav) hdr += '<th class="' + numClass + '">kW Savings ($)</th>';
  if (hasTotalCostSav) hdr += '<th class="' + numClass + '">Total Savings ($)</th>';
  hdr += '</tr>';
  ...
  rowData.forEach(function (d) {
    rowsHTML +=
      ...
      numClass +
      '" style="color:' +
      (d.isRegr ? violetColor : mutedColor) +
      '">' +
      fmtN(d.expUsage) +
      (d.isRegr ? '<span style="font-size:9px;color:' + violetColor + ';margin-left:3px">R</span>' : '') +
      '</td>' +
      '<td class="' +
      numClass +
      '">' +
  ...
  var sumRow =
    ...
    '<td class="' +
    lblClass +
    '" colspan="6" style="text-align:right;font-weight:800;text-transform:uppercase;font-size:10px;letter-spacing:.5px">Total</td>';
