...
function buildMeterPerfTableHTML(m, bills, incl, opts) {
  ...
  if (isGas) {
    ...
    allRows.forEach(function (r) {
      ...
      gasCostByYm[r.ym] = { thermRate: billRate || rowRate, thermCostAmt: r.cost || 0, ct: 1 };
    });
  }
  ...
  postRows.forEach(function (r) {
    ...
    // 2026-09-10 (zero-fill guard): a zeroFill row (propane, no delivery yet
    // this month) has real Baseline/expMo but no real Actual observation —
    // expMo - 0 would otherwise display a fabricated ~100% "Gal Saved"/"%"
    // for a month nothing has actually been measured on. The dollar Savings
    // column is already correctly $0/"—" here via the thermRate>0 guard
    // below; this keeps the unit columns consistent with that.
    if (r.zeroFill) {
      sav = 0;
      savPc = 0;
    }
